...
class TabProduct extends React.Component {
        ...
        if (e.key == 'Enter') {
            ...
            if (check.length == 0) {
                ...
                if (list_products_by_baogia.length == 0) {
                    if (objDataOrder.CodeBaoGia) {
                        this.props.getListProductsByPrice(objDataOrder.CodeBaoGia).then(() => {
                            setTimeout(() => {
    ...
    _funcAddExistProduct(id, value) {
        this.props.addExistProduct(value)
        setTimeout(() => {
            let { list_products, objDataOrder, listProductsSelected } = this.props.order
            let list_products_temp = _.clone(list_products, true)
                                ...
                                <tr key={`data_${i}`} id={`tr_${index}`}>
                                    <td>
                                        <div style={{ "border": "1px solid black", "display": "inline-block", "padding": "2px 2px" }}>
                                            <img src={filename} width="85px" height="85px" />
                                        </div>
                                    </td>
                                    <td>
                                        <input
                                    ...
                                    <td><button onClick={() => this._funcAddExistProduct(index, value)}><i className="fa fa-plus" aria-hidden="true"></i></button></td>
                                    <td>
                                        <Cell
                                            width="120px"
                                    ...
                                    <Cell type="price_basic" readOnly={true} width="100px" value={price_basic} keyInput="price_basic" id={index} parentObject={this} />
                                    <td width="100px">{SportConfig.function._formatMoney(sum_basic)}</td>
                                    <Cell type="price" readOnly={true} width="100px" value={Helper.roundNumberPerThousand(price)} keyInput="price" id={index} parentObject={this} />
