        ...
        if (objDataOutput.IdOutput) {
            return new Promise((resolve, reject) => {
                axios.get(`${Config.API_URL_USER}output/get_list_product_by_order`, { params: { IdOutput: objDataOutput.IdOutput } })
                    .then((response) => {
                        let { data } = response.data
                        let listProductsSelected_temp = _.clone(ListProductByOrderOutput, true);
                        let data_temp = data
                        // map data
...
export const selectCustomerInOutOut = (value) => {
    return (dispatch, getState) => {
        ...
        objDataOutput["IdCustomer"] = value.value
        objDataOutput["CodeKH"] = value.value
        objDataOutput["NameKH"] = value.label
        dispatch({
            type: SELECT_CUSTOMER_IN_OUTPUT,
            payload: {
                namecustomer: value.name,
                objDataOutput: objDataOutput
            }
        })
    }
}
...
export const getListDataBySearch = (value) => {
    return (dispatch, getState) => {
        return new Promise((resolve, reject) => {
            axios.get(`${Config.API_URL_USER}output/search`, { params: { page: 1, total: 10, key: value } })
                .then((response) => {
                    let { data } = response.data
                    let data_temp = data
...
export const printOuputDetail = (itemDetail, type) => {
    return (dispatch, getState) => {
        let { ListProductByOrderOutput } = getState().output
        return new Promise((resolve, reject) => {
            axios.get(`${Config.API_URL_USER}ouput/get_data_report_output`
                , { params: { IdOutput: itemDetail.IdOutput } })
                .then(res => {
                    let { data } = res
                    axios.post(`${Config.API_URL_USER}ouput/print_detail_output`,
                        ...
                        .then((response) => {
                            ...
                            if (type == 'type3') {
                                ...
                                link.setAttribute('download', `Xuất hàng_${itemDetail.IdOrder}`); //or any other extension
                            }
...
export const getListDataOutPut = (params = { page: 1, total: 10 }) => {
    return (dispatch, getState) => {
...
export const clickCheckRowOrder = (value, checked) => {
    return (dispatch, getState) => {
        ...
        setTimeout(() => {
            dispatch(getListProductsByOrderOutput(value.IdOrder)).then(() => {
                dispatch(getListProductByOrderInOutput())
            })
        }, 200)
    }
}
export const addProduct = () => {
    return (dispatch, getState) => {
        ...
        dispatch({
            ...
            payload: {
                ListProductByOrderOutput: sort_data
            }
        })
    }
}
    ...
    return (dispatch, getState) => {
        ...
        return new Promise((resolve, reject) => {
            Promise.all([
                ...
                axios.post(`${Config.API_URL_USER}output/add_list_products`, { data: data_temp, IdOutput: objDataOutput.IdOutput, IdOrder: objDataOutput.IdOrder })                    // axios.post(`${Config.API_URL_USER}products/add_list_casting`, { data: data_temp2 })
            ])
                .then((response) => {
                    dispatch({
...
export const clickCheckRowOrderInOutPut = (IdOrder, checked) => {
    return (dispatch, getState) => {
        ...
        dispatch(getListProductsByOrderOutput(IdOrder))
    }
}
...
export const getListDataOrderInOutPut = (params = { page: 1, total: 10 }) => {
    return (dispatch, getState) => {
...
export const getListProductsByOrderOutput = (IdOrder) => {
    return (dispatch, getState) => {
        ...
        return new Promise((resolve, rejects) => {
            axios.get(`${Config.API_URL_USER}ouput/output_get_product_by_order`, { params: { IdOrder: IdOrder } })
                .then((response) => {
                    let { data } = response.data
                    let data_temp = data
                    data_temp.map((item, i) => {
...
export const selectOrderOutPut = (obj) => {
    return (dispatch, getState) => {
        let { objDataOutput } = getState().output
        objDataOutput["IdOrder"] = obj.value
        dispatch({
        ...
        return new Promise((resolve, reject) => {
            axios.get(`${Config.API_URL_USER}output/get_auto_number`)
                .then((response) => {
                    let { data } = response.data
                    let { objDataOutput } = getState().output
                    objDataOutput["IdOutput"] = (data && data[0].value || '0000000')
                    dispatch({
        ...
        return new Promise((resolve, reject) => {
            axios.post(`${Config.API_URL_USER}order/accept_order`, { IdOrder: id, Value: value })
                .then((response) => {
                    let { data } = response.data
                    if (data != 2) {
                        let objDataOutput_temp = objDataOutput
