    ...
    return (dispatch, getState) => {
        ...
        dataTemp = dataTemp.map((item) => {
            if (item.IdProduct + item.Color == obj.id) {
                item.Qty = parseInt(obj.value)
                item.Value = parseInt(obj.value)
            }
            return item
        })
    ...
    return (dispatch, getState) => {
        ...
        dispatch({
            ...
            payload: {
                ...
                listItemCreateBagDefault: data_temp
            }
        })
    }
}
                    ...
                    list_data.map((itemProduct, ii) => {
                        ...
                        itemProduct.listColor.map((itemColor) => {
                            data.map((item, i) => {
                                ...
                                item_temp.IdProductColorParentColorStone = IdProductTemp + itemProduct.Color + item_temp.Color + item.value
...
export const getListStoneByProductsInBagNew = (list_data) => {
    return (dispatch, getState) => {
        return new Promise((resolve, reject) => {
    ...
    return (dispatch, getState) => {
        return new Promise((resolve, reject) => {
            axios.get(`${Config.API_URL_USER}bag/get_list_products_by_order_splitbag`, { params: { IdOrder: obj.IdOrder, IdProduct: obj.IdProduct, Color: obj.Color } })
                .then((response) => {
                    let { data } = response.data
                    data.map(x => x.SplitBag = 1)
                    // cập nhật số lượng qty
                    let listItemCreateBag_temp = data.map(item => {
