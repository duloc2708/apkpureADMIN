...
class FormSplitStoneUpdate extends React.Component {
    constructor() {
        super()
    }
    componentDidMount() {
    ...
    _onSplitStone() {
        ...
        if (value > 0) {
            ...
            if (TypeSplitStone == 'TYPE_SPLIT_STONE_1') {
                ...
                let valueTotal = parseInt(value) * parseInt(sumsTotalQty[0].ProductsEachBag)
                if (valueTotal > parseInt(itemStoneSplit.sl2)) {
                    this.child._addNotification('Số lượng đá đã hết!', 'warning')
                } else {
                    this.props.SplitStoneDetail(valueTotal)
                ...
                let valueTotal = parseInt(value) * parseInt(itemDetailCreateBag.ProductsEachBag)                
                if (valueTotal > parseInt(itemStoneSplit.sl2)) {
                    this.child._addNotification('Số lượng đá đã hết!', 'warning')
                } else {
                    this.props.SplitStoneDetail(valueTotal)
...
export default ReactRedux.connect(mapStateToProps, mapDispatchToProps)(FormSplitStoneUpdate)