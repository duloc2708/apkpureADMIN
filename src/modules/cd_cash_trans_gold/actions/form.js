...
export const sortDataListCash = obj => {
  return (dispatch, getState) => {
    let { listHeaderTable } = getState().cd_cash_trans_gold;
...
export const getListOutput = params => {
  return (dispatch, getState) => {
    return new Promise((resolve, reject) => {
      axios
        .get(`${Config.API_URL_USER}output`, {
...
export const deleteItem = params => {
  return dispatch => {
    return new Promise((resolve, reject) => {
      axios.post(`${Config.API_URL_USER}cash_trans/delete`, params).then(
        response => {
          resolve(response);
        },
        err => {
...
export const printDetail = item => {
  return (dispatch, getState) => {
    return new Promise((resolve, reject) => {
      axios
        .post(
          `${Config.API_URL_USER}cash_trans/print_detail`,
          { Info: item, type: 0 },
          {
...
export const updateListOutput = value => {
  return (dispatch, getState) => {
    let { listOutputDefault, objData } = getState().cd_cash_trans_gold;
    let listOutputDefaultTemp = _.clone(listOutputDefault, true);
    let objDataTemp = _.clone(objData, true);
    listOutputDefaultTemp = listOutputDefaultTemp.filter(
      x => x.IdCustomer == value
    );
...
export const selectOutputCash = value => {
  return (dispatch, getState) => {
    let { objData } = getState().cd_cash_trans_gold;
...
export const saveWeightStone = () => {
  return (dispatch, getState) => {
    return new Promise((resolve, reject) => {
      let { listStoneWaxset } = getState().cd_cash_trans_gold;
      axios
...
export const acceptStatus = id => {
  return (dispatch, getState) => {
    return new Promise((resolve, reject) => {
      axios
        .get(`${Config.API_URL_USER}cash_trans/accept_status`, {
          params: { IdTicket: id }
        })
        .then(
          response => {
...
export const completeTicket = id => {
  return (dispatch, getState) => {
    return new Promise((resolve, reject) => {
      axios
        .get(`${Config.API_URL_USER}cash_trans/complete_ticket`, {
          params: { IdTicket: id }
        })
        .then(
          response => {
      ...
      axios
        ...
        .then(
          response => {
            let { data } = response.data;
            data.map(item => {
              let itemExists = listStoneWaxsetTemp.filter(
                x => x.IdPrimaryStone == item.IdPrimaryStone
              );
...
export const getNumberAutoCashTrans = () => {
  return (dispatch, getState) => {
    return new Promise((resolve, reject) => {
      axios.get(`${Config.API_URL_USER}cash_trans/get_auto_number`).then(
        response => {
          let { data } = response.data;
          let { objData } = getState().cd_cash_trans_gold;
          let objData_temp = _.clone(objData, true);
...
export const addNewItem = data => {
  ...
  return (dispatch, getState) => {
    ...
    let { objData } = getState().cd_cash_trans_gold;
    objData["Payment_Type_Gold"] = "1";
    return new Promise(
      (resolve, reject) => {
        axios
          .post(`${Config.API_URL_USER}cash_trans/add`, {
            objData: objData
          })
          .then(response => {
            dispatch({
...
export const updateItem = data => {
  ...
  return (dispatch, getState) => {
    ...
    return new Promise(
      (resolve, reject) => {
        axios
          .post(`${Config.API_URL_USER}cash_trans/update`, {
            objData: objData
          })
          .then(response => {
            dispatch({
    ...
    return new Promise((resolve, reject) => {
      axios
        ...
        .then(
          response => {
            let { data } = response;
            let dataSakura = data.filter(x => x.Code == "SAJI00");
            console.log("dataSakura", dataSakura);

            dispatch({
              type: GET_LIST_CUSTOMER,
              payload: {
                listCustomer: data || [],
                listCustomerDefault: data || [],
                listCustomerSakura: dataSakura || []
              }
            });
            resolve(response);
          },
...
export const updateCustomerSakura = type => {
  return (dispatch, getState) => {
    let {
      listCustomerSakura,
      listCustomerDefault
    ...
    let { objData } = getState().cd_cash_trans_gold;
    objData["IdCustomer"] = "";
    if (type != "PAYMENT_CASH_GOLD_03") {
      dispatch({
        type: GET_LIST_CUSTOMER,
        payload: {
          objData: objData,
          listCustomer: listCustomerDefault || []
        }
      });
    } else {
      if (listCustomerSakura.length == 1) {
        objData["IdCustomer"] = listCustomerSakura[0].Code;
        objData["nameCustomer"] = listCustomerSakura[0].Name;
      }
      dispatch({
        type: GET_LIST_CUSTOMER,
        payload: {
          objData: objData,
          listCustomer: listCustomerSakura || []
        }
      });
    }
  };
...
export const initAddCastingProc = () => {
  return (dispatch, getState) => {
    let { listBagSelected, default_bag } = getState().cd_cash_trans_gold;
  ...
  return (dispatch, getState) => {
    ...
    temp.map((item, i) => {
      if (item.orderby == obj.id) {
        ...
        item["BrokenRate"] =
          valueTemp / (AvgStoneTemp * parseInt(QtyProduct || 0));
    ...
    listBagSelectedTemp.map(item => {
      if (item.IdBag == IdBagInStone) {
        ...
        if (typeInOut == "IN") {
          ...
          item["Gold_Weight_IN"] =
            valueTemp * item.Broken_Weight_IN - Waxset_WeightTemp;
        } else {
          let valueTemp = Product_Weight_OUT;
          item.Broken_Weight_OUT = totalWeightBrokenTemp;
          item["Gold_Weight_OUT"] =
            valueTemp * item.Broken_Weight_OUT - Waxset_WeightTemp;
        }
        if (IsIncludeInOut == 1) {
...
export const updateCellBag = obj => {
  return (dispatch, getState) => {
    let { listBagSelected, objConfig } = getState().cd_cash_trans_gold;
...
export const updateExistBag = data => {
  return (dispatch, getState) => {
    let { listBagSelected, list_products } = getState().cd_cash_trans_gold;
...
export const removeItemBag = obj => {
  return (dispatch, getState) => {
    let { listBagSelected } = getState().cd_cash_trans_gold;
    let listBagSelectedTemp = _.clone(listBagSelected, true);
...
export const clickCheckRowCasting = (value, checked) => {
  return (dispatch, getState) => {
    return new Promise((resolve, reject) => {
      let { list_data } = getState().cd_cash_trans_gold;
...
export const getRemainingDebt = () => {
  return (dispatch, getState) => {
    ...
    return new Promise((resolve, reject) => {
      axios
        .get(`${Config.API_URL_USER}cash_trans/get_remmainig_debt`, {
          params: { IdCustomer: objData.IdCustomer }
        })
        .then(
          response => {
            let { data } = response.data;
            let objDataTemp = _.clone(objData, true);
            objDataTemp["Remaining_Debt"] =
              (data[0] && data[0].Open_Balance) || 0;
            dispatch({
              type: UPDATE_CELL_INPUT_CASH,
              payload: {
                objData: objDataTemp
...
export const getListDataCashTrans = (
  value = "",
  sortKey = "",
  sortBy = "",
  keyFilter = "",
  ...
  return (dispatch, getState) => {
    ...
    return new Promise((resolve, reject) => {
      axios
        .get(`${Config.API_URL_USER}cash_trans_gold`, {
          params: {
            page: page,
            total: total,
            key: value,
        ...
        .then(
          response => {
            ...
            data_temp = data_temp.filter(x => x.Payment_Type_Gold != null);
            dispatch({
...
export const getListBagInCastingProc = (value = "") => {
  return (dispatch, getState) => {
    return new Promise((resolve, reject) => {
      axios.get(`${Config.API_URL_USER}ticket_proc/get_all_bag`).then(
        response => {
          let { data } = response.data;
          let data_temp = data;
          dispatch({
