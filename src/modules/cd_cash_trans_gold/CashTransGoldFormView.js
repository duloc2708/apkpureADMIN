...
import * as cashCashTransActions from 'modules/cd_cash_trans_gold/actions/form'
import BrackcrumFromView from 'modules/brackcrum/BrackcrumFromView'

class CashTransGoldFormView extends React.Component {
    constructor() {
        super()
        this.state = {
            inputSearch: ''
    ...
    _validateSave() {
        let { objData, listBagSelected, isSave } = this.props.cd_cash_trans_gold
        ...
        if (status == 'EDIT' && (['STATUS_TRANS_02', 'STATUS_TRANS_04'].indexOf(objData.Status) != -1)) {
            checktype = 2
        }
        if (!objData.PaymentAmount || objData.PaymentAmount == '0') {
            checktype = 3
        }
        if (!objData.Payment_Type) {
            checktype = 4
        }
        ...
        if ([1, 2, 3, 4, 5].indexOf(checktype) != -1) {
            return false
        }
        return true
    }
    ChangeButton(value) {
        ...
        let { objData, listBagSelected, isSave } = this.props.cd_cash_trans_gold
        let { status } = this.props.toolbar
        if (objData.status == 'STATUS_TRANS_04') {
            this.child._addNotification('Phiếu đã này đã hoàn thành', 'warning');
            return false
        }
        if (isSave) {
            this.child._addNotification('Dữ liệu hệ thống đang lưu, vui lòng đợi!')
        } else {
            switch (valueTemp) {
                case "PRINT":
                    break;
    ...
    render() {
        let { isDetail } = this.props.cd_cash_trans_gold
...
export default ReactRedux.connect(mapStateToProps, mapDispatchToProps)(CashTransGoldFormView)
