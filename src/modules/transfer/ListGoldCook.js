...
class ListGoldCook extends React.Component {
  ...
  render() {
    ...
    let blockInput =
      [STATUS_TRANS_04, STATUS_TRANS_02].indexOf(Status) != -1 ? true : false;

    if (objSearchGold.valueLV) {
      listGoldCookTemp = listGoldCookTemp.filter(
        x => x.ValueLV.toString().indexOf(objSearchGold.valueLV) !== -1
      );
    }
    return (
      <div>
          ...
          <tbody>
            {listGoldCookTemp &&
              listGoldCookTemp.map((item, i) => {
                ...
                let calTotalGoldCook = TotalWeightAfterWarm
                  ? Helper.round(
                      (ValueLV * TotalWeightAfterWarm) / totalGoldCook,
                      4
                    )
                  : 0;
                  ...
                  <tr key={`data_${i}`}>
                    <td>
                      <input
                        ...
                        type="checkbox"
                        onChange={() =>
                          this.props.changeStatusInput(IdStore, checked)
                        }
                      />
                    </td>
                    <td>{IdStore}</td>
            ...
            <tr>
              ...
              <td></td>
              <td>
                <b>Tổng TL vàng quy 10</b>
              </td>
              <td>{Helper.round(totalTF_Weight10, 4)}</td>
              <td>
                <b>Tổng TL vàng sau nấu:</b>{" "}
                {Helper.round(totalWeightAfterCook, 4)}
              </td>
            </tr>
          </tbody>
        </table>
...
const mapStateToProps = ({ userAuth, i18n, transfer }, ownProps) => {
  return {
    userAuth,
    i18n,
    ownProps,
