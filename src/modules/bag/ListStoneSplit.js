...
class ListStoneSplit extends React.Component {
  ...
  _changeHandset(item, status) {
    let itemTemp = _.clone(item, true);
    itemTemp.isHanset = status;
    this.props.updateHandsetStoneSplit(itemTemp);
  }
  render() {
    ...
    let IdProduct_temp =
      itemDetailCreateBag.IdOdd == "1"
        ? itemDetailCreateBag.IdChildrenProduct
        : itemDetailCreateBag.IdProduct;
    list_stone_save_split = list_stone_save_split.filter(
      item =>
        item.IdProductColorParentColorStone ==
        itemStoneSplit.IdProductColorParentColorStone
    );
    return (
      <div>
        <table className="table table-striped" id={idTable}>
          <thead>
            <tr>
              <th style={{ textAlign: "left" }} key={`thead_${1}`} scope="col">
                {"Số lượng"}
              </th>
              <th style={{ textAlign: "left" }} key={`thead_${13}`} scope="col">
                {"Đá handset"}
              </th>
              <th style={{ textAlign: "left" }} key={`thead_${2}`} scope="col">
                {"Mã"}
              </th>
              <th style={{ textAlign: "left" }} key={`thead_${3}`} scope="col">
                {"Màu"}
              </th>
              <th style={{ textAlign: "left" }} key={`thead_${4}`} scope="col">
                {"Loại đá"}
              </th>
              ...
              <th style={{ textAlign: "left" }} key={`thead_${7}`} scope="col">
                {"TL TB"}
              </th>
              <th style={{ textAlign: "left" }} key={`thead_${8}`} scope="col">
                {"TLTB t.khảo"}
              </th>
          ...
          <tbody>
            {list_stone_save_split.map((item, i) => {
              let {
                ...
                IdProduct,
                IdProductColorStone,
                IdProductColorParentColorStone,
                ColorParent,
                IdProductParent,
                sl2,
                Exchange,
              ...
              return (
                <tr key={`dataDetail_${i}`}>
                  <td
                    style={{
                      textAlign: "left",
                      fontWeight: PrimaryStone == 1 ? "bold" : ""
                    }}
                  >
                    {sl2 || ""}
                  </td>
                  ...
                  <td style={{ textAlign: "left" }}>
                    <input
                      checked={isHanset ? true : false}
                      onChange={() => this._changeHandset(item, !isHanset)}
                      type="checkbox"
                      checked={isHanset == 1 ? true : false}
                    />
                  </td>
                  <td
                    style={{
                  ...
                  <td style={{ textAlign: "left" }}>
                    <Combobox
                      ...
                      id={
                        IdProductParent + ColorParent + IdStone + "/" + IdStone
                      }
                      parentObject={this}
                    />
                  </td>
                  <td style={{ textAlign: "left" }}>
                    <Combobox
                      disable={false}
                      type_code="TYPE_STONE"
                      keyInput="TypeStone"
                      value={TypeStone ? TypeStone : "TYPE_STONE_1"}
                      id={SplitIdProductColorParentColorStone}
                      parentObject={this}
                    />
                  </td>
                  <Cell
...
const mapStateToProps = ({ userAuth, i18n, order, header, bag }, ownProps) => {
  return {
    userAuth,
    i18n,
    ownProps,
