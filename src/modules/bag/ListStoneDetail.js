...
import FormSplitStoneDetail from './FormSplitStoneDetail';
const customStyles = {
    content: {
        top: '50%',
        left: '50%',
...
class ListStoneDetail extends React.Component {
    constructor() {
        super()
        this.state = {
            code: '',
                ...
                <table className="table table-striped">
                    <thead>
                        <tr>
                            ...
                            <th style={{ "textAlign": "left" }} key={`thead_${7}`} scope="col">{'TL TB'}</th>
                            <th style={{ "textAlign": "left" }} key={`thead_${8}`} scope="col">{'TLTB t.khảo'}</th>
                            <th style={{ "textAlign": "left" }} key={`thead_${11}`} scope="col">{'Tách'}</th>
                    ...
                    <tbody>
                        {list_stone_save.map((item, i) => {
                            let { IdStone, Color, numofstone, value, Weight, PrimaryStone, IdProduct, IdProductColorStone, IdProductColorParentColorStone, 
                                ColorParent, IdProductParent, sl2, exchange, TypeStone, AvgStone, IdProductParentIdProductStone,sl,AvgWeight } = item
                            AvgStone = Weight / sl2
                            return (
                                <tr key={`dataDetail_${i}`}>
                                    ...
                                    <td><input type="checkbox" checked={PrimaryStone ==1? true : false} /></td>
                                    <td style={{ "textAlign": "left", "fontWeight": PrimaryStone == 1 ? 'bold' : '' }}>
                                        {IdStone}
                                    </td>
                                    ...
                                    <Cell type="text" width="100px" value={Weight} keyInput="value" id={itemDetailCreateBag.IdOdd == "1" ? IdProductParentIdProductStone : IdProductColorStone} parentObject={this} />
                                    <td >
                                        {exchange}
                                    </td>
                                    <td style={{ "textAlign": "left" }}>
...
export default ReactRedux.connect(mapStateToProps, mapDispatchToProps)(ListStoneDetail)
