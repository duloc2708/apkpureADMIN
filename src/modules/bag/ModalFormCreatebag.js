...
class TabOrder extends React.Component {
    ...
    ChangeValueComboboxColor(obj) {
        let { listItemCreateBagDefault, itemDetailCreateBag } = this.props.bag

        let findItem = listItemCreateBagDefault.filter(x => x.Color == obj.value)
        this.props.getListProductsByOrderSplitBag(findItem[0], findItem[0].Qty)

        let objItem = _.clone(itemDetailCreateBag, true)
        objItem.Color = obj.value
        objItem.ColorParent = obj.value
        objItem.IdProductColor = objItem.IdProduct + obj.value

        this.props.updateItemDetailBag(objItem)
    }
    handleClick() {
    ...
    _updateEditBagNo() {
        this.props.updateEditBagNo()
    }
    handleChangeBagNo(e) {
        let { value } = e.target
        this.props.changeBagNo(value)
    }
    render() {
        let { listItemCreateBag, itemDetailCreateBag, editBagNo } = this.props.bag
                ...
                <div className="row">
                    <div className="col-md-3">
                        <div className="form-group">
                            ...
                            <div className="right" >
                                <input readOnly={editBagNo}
                                    onChange={(e) => this.handleChangeBagNo(e)}
                                    className="name form-control" value={listItemCreateBag[0] && listItemCreateBag[0].Id || ''} type="text" id="IdOrder" name="name" required=""
                                />
                                <button onClick={() => this._updateEditBagNo()}><i className="fa fa-pencil-square-o" aria-hidden="true"></i></button>                            </div>
                        </div>
                    </div>
                    <div className="col-md-3">
                        <div className="form-group">
                                ...
                                <table id={idTable} className="table table-striped">
                                    ...
                                    <tbody>
                                        {listItemCreateBag.map((item, i) => {
                                            let { Color, IdOrderProduct, Value, IdProduct, ProductsEachBag, Number, IdOdd, listChildrenProduct, IdChildrenProduct, IdProductColor, isDelete, listColor, Qty } = item
                                            let { list_stone_save_split } = this.props.bag
                                            ...
                                            return (
                                                <tr key={`dataDetail_${i}`} style={styles} >
                                                    ...
                                                    <td style={{ "textAlign": "center" }}>
                                                        {
                                                            // sử dụng trong trường hợp tách nhiều màu 
                                                            listColor && listColor.length > 1 ?
                                                                <ComboboxColor data_filter={listColor} type_code='DSM' keyInput="Color" value={Color} id={IdOrderProduct || '1'} parentObject={this} />
                                                                :
                                                                <Combobox type_code='DSM' keyInput="Color" disable={true} value={Color} id={IdOrderProduct || '1'} parentObject={this} />
                                                        }
                                                    </td>
                                                    {/* {
                                                        // sử dụng trong trường hợp tách nhiều màu 
                                                        listColor && listColor.length > 1 ?
                                                            ...
                                                            :
                                                            <Cell idTable={idTable} index={i} type="number" width="100px" other={{ IdProduct: IdProduct, Color: Color }} value={parseInt(Value) || ''} keyInput="Value" id={IdOrderProduct} parentObject={this} />
                                                    } */}
                                                    <Cell idTable={idTable} index={i} type="number" readOnly={true} width="100px" other={{ IdProduct: IdProduct, Color: Color }} value={parseInt(Value) || ''} keyInput="Value" id={IdOrderProduct} parentObject={this} />
                                                    <td style={{ "textAlign": "center" }}>
                                                        {Number}
                                                    </td>
                                                    <td style={{ "textAlign": "center" }}>
