...
export const getBagNoExist = (IdBag) => {
    return (dispatch, getState) => {
        let { listItemCreateBag } = getState().bag
        return new Promise((resolve, reject) => {
            axios.get(`${Config.API_URL_USER}bag/get_bag_no_exists`, { params: { IdBag: listItemCreateBag[0].Id } })
                .then(res => {
                    ...
                    if (value == 1) {
                        dispatch({
                            ...
                            payload: {
                                IdBagCreate: listItemCreateBag[0].Id
                            }
                        })
                    }
                    resolve(res)
...
export const changeBagNo = (value) => {
    return (dispatch, getState) => {
        let { listItemCreateBag } = getState().bag
        let listItemCreateBagClone = _.clone(listItemCreateBag, true)
        listItemCreateBagClone.map(x => x.Id = value)
        return new Promise((resolve, reject) => {
            dispatch({
...
export const cancelBag = (item) => {
    return (dispatch, getState) => {
        return new Promise((resolve, reject) => {
            axios.get(`${Config.API_URL_USER}bag/cancel`, { params: { IdBag: item.Id, IdOrder: item.IdOrder, username: username } })
                .then(res => {
                    resolve(res)
                })
        })
...
export const deleteBag = (item) => {
    return (dispatch, getState) => {
        return new Promise((resolve, reject) => {
            axios.get(`${Config.API_URL_USER}bag/delete`, { params: { IdBag: item.Id, IdOrder: item.IdOrder, username: username } })
                .then(res => {
                    resolve(res)
                })
        })
    ...
    return (dispatch, getState) => {
        ...
        if (itemDetailCreateBag.IdOdd != "1") {
            ...
            list_stone_save_temp = list_stone_save_temp.map((item) => {
                ...
                if (item.IdProductParentIdProductStoneColor == itemCheckClone.IdProductParentIdProductStoneColor) {
                    if (PrimaryStone == 1) {
...
export const updateHandsetStoneSplit = (objTemp) => {
    return (dispatch, getState) => {
        let { list_stone_save_split } = getState().bag
        let temp = _.clone(list_stone_save_split, true);
        temp.map((item, i) => {
            if (item.SplitIdProductColorParentColorStone == objTemp.SplitIdProductColorParentColorStone) {
                item["isHanset"] = objTemp.isHanset
            }
            return item
        })
        dispatch({
...
export const UpdateBagList = (StatusWeight) => {
    return (dispatch, getState) => {
        ...
        return new Promise((resolve, reject) => {
            Promise.all([
                axios.post(`${Config.API_URL_USER}bag/insert_bag_new`, { listBagApi: listBagApi, isEdit: 1 })
            ])
                .then((response) => {
                    dispatch({
                        type: LOADING_BAG,
...
export const InsertBagListSplitOdd = (listInsertBagTemp) => {
    ...
    return (dispatch, getState) => {
        ...
        return new Promise((resolve, reject) => {
            axios.post(`${Config.API_URL_USER}bag/insert_bag_new`, { listBagApi: listBagApi, isEdit: 0 }).then((response) => {
                dispatch({
                    type: SAVE_BAG,
                    payload: {
                        isSaveBag: true,
...
export const InsertBagList = (listItemCreateBagTemp) => {
    ...
    return (dispatch, getState) => {
        ...
        list_stone_save_temp.map(x => x.created_by = username)
        list_stone_split_save_temp.map(x => x.created_by = username)
        list_stone_save_temp.map(x => x.IdBag = Id)
        list_stone_split_save_temp.map(x => x.IdBag = Id)
        list_stone_split_save_custom_temp.map(x => x.IdBag = Id)
        listBagApi.push({
            dataBag: listItemCreateBag_temp,
            dataStone: list_stone_save_temp,
            dataStoneSplit: list_stone_split_save_temp,
