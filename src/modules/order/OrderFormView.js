...
class ProductFormView extends React.Component {
  ...
  _validateSave() {
    ...
    if (objDataOrder.CodeBaoGia == "" || objDataOrder.CodeBaoGia == undefined) {
      this.child._addNotification("Khách hàng chưa có bảng giá", "warning");
      return false;
    }
    ...
    listProductsSelected.map(item => {
      ...
      if (size == "" && value) {
        if (["B", "R"].indexOf(CodeCLSP) == -1) {
          this.child._addNotification(
            `Vui lòng nhập size cho sản phẩm ${value} có chủng loại ${NameCLSP}`,
            "warning"
          );
          check = false;
          return false;
      ...
      if (color == "") {
        this.child._addNotification(
          `Vui lòng nhập màu cho sản phẩm ${value}`,
          "warning"
        );
        check = false;
        return false;
      ...
      switch (valueTemp) {
          ...
          if (this._validateSave() && this.checkSameProducts()) {
            if (status == "EDIT") {
              ...
              this.props.getNumberAutoOrder().then(data => {
                this.props.addNewItemOrder().then(res => {
        ...
        case "DELETE":
          ...
          if (objDataOrder && objDataOrder.IdOrder) {
            ...
            if (r == true) {
              this.props
                .deleteItemOrder({ id: objDataOrder.IdOrder })
                .then(res => {
            ...
            this.child._addNotification(
              I18n.t(`alert.please_select_delete`),
              "warning"
            );
          }
          break;
  ...
  checkSameProducts() {
    ...
    sumsProducts.map(item => {
      if (item.countProducts > 1) {
        ...
        let data_temp =
          list_data_all && list_data_all.filter(x => x.type_code == "DSM");
      ...
      <div className="container">
        ...
        <section>
          ...
          <div className="main__content">
            ...
            {isDetail ? <DetailFormView /> : <ListFormView />}
          </div>
        </section>
      </div>
    );
...
const mapStateToProps = (
  { userAuth, i18n, list, header, order, toolbar },
  ownProps
) => {
  return {
    userAuth,
