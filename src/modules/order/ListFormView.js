...
class ListProductFormView extends React.Component {
  ...
  _copyOrder(item) {
    var r = confirm(`Bạn có muốn copy đơn hàng ${item.IdOrder} ?`);
    if (r == true) {
      this.props.copyOrder(item).then(res => {
        const { objDataOrder } = this.props.order;
        this._EditOrder(objDataOrder);
                ...
                return (
                  ...
                  >
                    ...
                    <ButtonPermission
                      ...
                      icon={`fa fa-check`}
                      data={{ item: item, status: "STATUS_ORDER_02" }}
                      parentObject={this}
                    />
                    {pageRoute != "order" ? (
                      <td style={{ textAlign: "left" }}>
                        <button onClick={() => this._acceptOrderMold(item)}>
                    ...
                    <ButtonPermission
                      type="CANCEL_ORDER"
                      key="CANCEL_ORDER"
                      nameBtn="CANCEL_ORDER"
                      icon={`fa fa-ban`}
                      data={{ item: item, status: "STATUS_ORDER_03" }}
                      parentObject={this}
                    />
                    <ButtonPermission
                      type="EDIT"
                    ...
                    <ButtonPermission
                      ...
                      nameBtn="PRINT"
                      icon={`fa fa-print`}
                      data={{ item: item, typePrint: "type1" }}
                      parentObject={this}
                    />
                    <ButtonPermission
                      key="PRINT2"
                      ...
                      nameBtn="PRINT"
                      icon={`fa fa-print`}
                      data={{ item: item, typePrint: "type2" }}
                      parentObject={this}
                    />
                    <ButtonPermission
                      key="COPY"
...
const mapStateToProps = (
  { userAuth, i18n, order, header, common, routing },
  ownProps
) => {
  return {
    userAuth,
