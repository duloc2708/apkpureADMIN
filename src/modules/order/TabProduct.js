...
class TabProduct extends React.Component {
  ...
  _loadData() {
    ...
    var that = this;
    let { objDataOrder, isCopy } = this.props.order;
    if (objDataOrder.IdOrder && !isCopy ) {
  ...
  _onRemove(item) {
    let { isEditProducts } = this.props.order;

    if (isEditProducts != "block") {
      var r = confirm(I18n.t(`alert.delete`));
      if (r == true) {
        this.props.removeItemStone(item);
      }
    } else alert("Không thể sửa đơn hàng khi đã xác nhận");
  }
  ...
  _onKeyPressCheckProduct(e, IdProduct, index) {
    if (e.key == "Enter") {
      ...
      let check = list_products_temp.filter(
        x => x.value.toUpperCase() === IdProduct.toUpperCase()
      );
      ...
      if (check.length == 0) {
        ...
        if (check[0] && check[0].StatusOdd != null) {
          this.child._addNotification(
            `Sản phẩm ${check[0].StatusOdd} đã ngừng kinh doanh`,
            "warning"
          );
        }
  ...
  _onButtonAddProduct(index) {
    ...
    if (isEditProducts != "block" || isCopy) {
      ...
      if (
        !objDataOrder.CodeBaoGia ||
        (CodeBaoGiaTemp && CodeBaoGiaTemp.indexOf(",") != -1 && status == "ADD")
      ) {
        alert("Vui lòng chọn bảng giá");
        return;
      }
        ...
        <table className="table table-striped">
          ...
          <tbody id="table_product">
                ...
                return (
                  <tr key={`data_${i}`} id={`tr_${index}`}>
                    ...
                    <td>
                      <input
                        onKeyDown={e =>
                          this._onKeyPressCheckProduct(e, value, index)
                        }
                        readOnly={
                          price > 0
                            ? true
                    ...
                    <td width="120px">
                      {Helper.round(WeightReal ? WeightReal : Weight || 0, 4)}
                    </td>
                    <td width="100px">
                      {Helper.round(
                        WeightReal ? WeightReal * sl : Weight * sl || 0,
                        4
                      )}
                    </td>
                    <Cell
                      ...
                      readOnly={true}
                      width="100px"
                      value={SportConfig.function._formatMoney(price_basic)}
                      keyInput="price_basic"
                      id={index}
                      parentObject={this}
                    />
                    <td width="90px">
                      {SportConfig.function._formatMoney(sum_basic)}
                    </td>
                    <Cell
                      type="price"
                      readOnly={true}
                      width="100px"
                      value={SportConfig.function._formatMoney(
                        Helper.roundNumberPerThousand(price)
                      )}
                      keyInput="price"
                      id={index}
                      parentObject={this}
                    />
                    <td>
                      {SportConfig.function._formatMoney(
                        Helper.roundNumberPerThousand(sum)
                      )}
                    </td>
                    {checkbag ? (
                      ""
            ...
            <tr>
              ...
              <td></td>
              <td width="100px">
                <b>Tổng TL Vàng (Chỉ)</b>:
              </td>
              <td></td>
              <td>
                <b> {Helper.round(totalgoldweight, 4)}</b>
              </td>
              <td>
                <b> Tổng thành tiền</b>:
              </td>
...
const mapStateToProps = (
  { userAuth, i18n, stone, header, products, order, toolbar },
  ownProps
) => {
  return {
    userAuth,
