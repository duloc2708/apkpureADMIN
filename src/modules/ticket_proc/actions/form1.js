        ...
        axios
          ...
          .then(
            response => {
              ...
              let IdProductTemp = "";
              dataNew.map(item => {
                if (IdProductTemp && IdProductTemp != item.IdProduct) {
                  i += 1;
                }
                item.isColor = i;
              ...
              // clear stone Weight
              let listBagSelectedTemp = _.clone(listBagSelected, true);
              listBagSelectedTemp.map(x => (x.Broken_Weight_OUT = null));
    ...
    return new Promise((resolve, reject) => {
      ...
      temp.map((item, i) => {
        if (parseInt(item.orderby) === parseInt(obj.id)) {
          ...
          valueTemp = parseFloat(valueTemp || 0);
          switch (obj.key) {
            //Trường hợp IsInputweightbyM_OUT = 1 và IsSameWeight_OUT=1 thì khi user nhập Product_Weight_OUT_T thì tự động gán cho Gold_Weight_OUT_T hoặc ngược lại
            case "Product_Weight_OUT":
              alert("aaaa.?");
              if (IsInputweightbyM_OUT == 1 && IsSameWeight_OUT == 1) {
                item["Gold_Weight_OUT"] = valueTemp;
              } else {
            ...
            case "Product_Weight_IN":
              //Trường hợp IsInputweightbyM_IN = 1 và IsSameWeight_IN=1
              //thì khi user nhập Product_Weight_IN_T thì tự động gán cho Gold_Weight_IN_T
              if (IsInputweightbyM_IN == 1 && IsSameWeight_IN == 1) {
                item["Gold_Weight_IN"] = Helper.round(valueTemp, 4);
              } else {
                // (TL (Vàng + Đá) Trừ Đá Rớt + Tổng TL đá rớt ) - trọng lượng waxset
                item["Gold_Weight_IN"] = Helper.round(
