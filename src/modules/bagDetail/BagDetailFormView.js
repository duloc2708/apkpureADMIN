...
class BagDetailFormView extends React.Component {
  componentDidMount() {
    ...
    let list_bag_temp = [];
    list_bag.map(item => {
      if (item) list_bag_temp.push(item);
    });
  ...
  _onPhantom(type) {
    ...
    list_report_bag &&
      list_report_bag.map((item, i) => {
        ...
        let list_stone_temp = "";
        let list_body = [];
        let get_list_stone_detail = this._parseListStone(
          item.get_list_stone_detail,
          list_stone_by_color
        );
        let list_name_stone = [];
        ...
        get_list_stone_detail.map((item, i) => {
          ...
          if (IdProduct_temp != item.IdProduct) {
            if (
              i > 1 &&
              list_value_color_header != null &&
              list_value_color_header.length > 4
            ) {
              list_name_stone.push(["\n"]);
            }
        ...
        get_list_stone_detail.map((item2, ii) => {
          ...
          if (findNextProduct) {
            if (
              IdProduct_temp2 &&
              IdProduct_temp2 != findNextProduct.IdProduct
            ) {
              let arr_td_value = [],
                arr_td_value_CHI = [];
              arr.push(
                <tr
                  ...
                  key={`tr_data_total_${item2.Weight}`}
                >
                  {list_stone_by_color &&
                    list_stone_by_color.map((item, i) => {
                      let Weight_Temp = 0;
                      ...
                      if (Weight_Temp && item.ColorCode != "001") {
                        arrTotalweight[i] =
                          parseFloat(arrTotalweight[i] || 0) +
                          parseFloat(Weight_Temp);
                        // console.log('>>>>>>>>>> truongnn', item.ColorName + ':' + Weight_Temp  + ':' + arrTotalweight[i] + '-' + i);
                        if (
                          list_value_color_header != null &&
                          list_value_color_header.length < 5
                        ) {
                          arr_td_value.push(
                            "(" +
                              this.round(Weight_Temp, 4) +
                              ")" +
                              this.round(
                                Weight_Temp / (arrayQtyPerColor[i] * 0.0375),
                                1
                              )
                          );
                        } else {
          ...
          arr.push(
            <tr style={{ textAlign: "center" }} key={`tr_data_${item2.Weight}`}>
              {list_stone_by_color &&
                list_stone_by_color.map((item, i) => {
                  let arr_temp = [];

                  let check_arr_total_by_product = arr_total_by_product.filter(
                    x => x.IdProduct == item2.IdProduct
                  );
                  if (check_arr_total_by_product.length == 0) {
                    item2.arr_color.map(itemColor => {
                      arr_total_by_product.push({
                  ...
                  item2.arr_color.map(itemColor => {
                    if (itemColor.Color == item.ColorCode) {
                      ...
                      Weight_Temp = itemColor.Weightperqty;
                      arr_td_value2.push(
                        (Weight_Temp && this.round(Weight_Temp, 4)) || ""
                      );
                      arr_total_by_product.map(item_arr_total => {
                        if (
                          item_arr_total.Color == itemColor.Color &&
                          item_arr_total.IdProduct == item2.IdProduct
                        ) {
                          item_arr_total.Weight =
                            item_arr_total.Weight + itemColor.Weight;
                          item_arr_total.Weightperqty =
                            item_arr_total.Weightperqty +
                            itemColor.Weightperqty;
                        }
                      });
                      return;
                    }
              ...
              >
                {list_stone_by_color &&
                  list_stone_by_color.map((item, i) => {
                    ...
                    arr_total_by_product.map(itemColor => {
                      if (
                        itemColor.Color == item.ColorCode &&
                        itemColor.IdProduct == item2.IdProduct
                      ) {
                        Weight_Temp = itemColor.Weightperqty;
                      }
                    });
                    ...
                    if (Weight_Temp && item.ColorCode != "001") {
                      ...
                      arrTotalweight[i] =
                        arrTotalweight[i] + parseFloat(Weight_Temp);
                      //  console.log('>>>>>>>>>> 1truongnn', item.ColorName + ':' + Weight_Temp  + ':' + arrTotalweight[i] + '-' + i);
                      arr_td_value5.push(
                        "[" +
                          this.round(arrTotalweight[i], 4) +
                          "]" +
                          this.round(
                            arrTotalweight[i] / (arrayQtyPerColor[i] * 0.0375),
                            1
                          )
                      );
                    } else if (Weight_Temp && item.ColorCode == "001") {
                        ...
                        table: {
                          ...
                          body: [
                            [
                              ...
                              {
                                image:
                                  type_bag == "STATUS_BAG_PRODUCT_02"
                                    ? imgCheck
                                    : imgUnCheck,
                                width: 9,
                                height: 10,
  ...
  _parseListStone(get_list_stone_detail, list_stone_by_color) {
    ...
    get_list_stone_detail.map(item => {
      if (item.Total) {
        let listStoneSame = get_list_stone_detail.filter(
          x => x.Total == item.Total && x.IdProduct == item.IdProduct
        );
        if (
          arr_temp.filter(
            x => x.Total == item.Total && x.IdProduct == item.IdProduct
          ...
          listStoneSame.map(item => {
            ...
            arr_color.push({
              Color: item.ordercolor,
              Weight: item.Weight,
              Weightperqty: item.Weightperqty
            });
          });
    ...
    arr_temp.map(item => {
      let check = get_list_stone_detail.filter(
        x => x.Total == item.Total && x.issplit == 1
      );
      if (check.length > 0) {
        item.issplit = 1;
      }
              ...
              <tr>
                ...
                <td
                  style={{
                    ...
                    verticalAlign: "top",
                    width: `${checkSplitStone.length > 0 ? "600px" : "500px"}`
                  }}
                >
                    ...
                    <tbody>
                      <tr style={{ fontSize: "14px" }}>
                        ...
                        >
                          ...
                          >
                            <img
                              src={`data:image/jpeg;base64,${url_image}`}
                              width="90"
                              height="95"
                            />
                          </div>
                      ...
                      <tr>
                        <td>
                          <strong>Casting No:</strong>
                        </td>
                        <td>
                          Ngày tạo:{" "}
                          <strong>
                            {moment.utc(DateCreated).format("DD/MM/YYYY")}
                          </strong>
                        </td>
                      </tr>
                      ...
                      <tr
                        style={{
                          fontSize: `${
                            list_stone_by_color.length >= 4 ? "12px" : "14px"
                          }`
                        }}
                      >
                        <td
                          ...
                          style={{
                            width: `${
                              checkSplitStone.length >= 1 ? "300px" : "200px"
                            }`
                          }}
                        >
                          <span>
                          ...
                          {get_list_stone_detail.map((item, i) => {
                            let arr = [];
                            let check_temp = get_list_stone_detail.filter(
                            ...
                            if (findNextProduct) {
                              if (
                                IdProduct_temp &&
                                IdProduct_temp != findNextProduct.IdProduct
                              ) {
                                arr.push([
                                  <span key={`total_stone_${i}`}>{""} </span>,
                                  <br />
                                ]);
                              }
                            }
                              ...
                              <thead>
                                <tr>
                                  {list_stone_by_color &&
                                    list_stone_by_color.map((item, i) => {
                                      return (
                                        <th key={`th_stone_${item.ColorCode}`}>
                                          {item.ColorName}
                                        </th>
                                      );
                                    })}
                                  ...
                                  if (findNextProduct) {
                                    if (
                                      ...
                                      arr.push(
                                        ...
                                        >
                                          {list_stone_by_color &&
                                            list_stone_by_color.map(
                                              (item, i) => {
                                                let Weight_Temp = 0;
                                                arr_total_by_product.map(
                                                  itemColor => {
                                                    if (
                                                      itemColor.Color ==
                                                      item.ColorCode
                                    ...
                                    >
                                      {list_stone_by_color &&
                                        list_stone_by_color.map((item, i) => {
                                          ...
                                          if (
                                            check_arr_total_by_product.length ==
                                            0
                                          ) {
                                            item2.arr_color.map(itemColor => {
                                              arr_total_by_product.push({
                                          ...
                                          item2.arr_color.map(itemColor => {
                                            if (
                                              ...
                                              arr_total_by_product.map(
                                                item_arr_total => {
                                                  if (
                                                    item_arr_total.Color ==
                                                      itemColor.Color &&
                                                    item_arr_total.IdProduct ==
                                                      item2.IdProduct
                                                  ) {
                                                    item_arr_total.Weight =
                                                      item_arr_total.Weight +
                ...
                <td
                  style={{
                    ...
                    fontWeight: "normal",
                    width: `${
                      list_stone_by_color.length >= 4 ? "290px" : "301px"
                    }`,
                    fontSize: `${
                      list_stone_by_color.length >= 4 ? "13px" : "13px"
                    }`
                  }}
                >
                  <table className="table head">
                    <thead>
                      <tr>
                        ...
                        >
                          <strong>Tổng TL đá rớt (Gram)</strong>
                        </th>
                        <th
                          className="small-width"
                          style={{
                ...
                <div className="col-md-12">
                  <div className="form-group ">
                    <div style={{ paddingTop: "10px" }}>
                      ...
                      <a style={{ margin: "2px" }}>
                        <button onClick={() => this._onDown("A4")}>
                          PRINT (A4)
                        </button>
                      </a>
                      <a style={{ margin: "2px" }}>
                        <button onClick={() => this._onDown("A5")}>
                          PRINT (A5)
                        </button>
                      </a>
                    </div>
...
const mapStateToProps = (
  { userAuth, i18n, order, header, bag, bagDetail },
  ownProps
) => {
  return {
    userAuth,
