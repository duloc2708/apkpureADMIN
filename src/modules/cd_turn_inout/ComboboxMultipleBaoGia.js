...
class ComboboxMultipleBaoGia extends React.Component {
    ...
    componentWillReceiveProps(nextProps) {
        let { value, type_code, Customer, id, comboOther, list_data_other } = this.props
        let { Customer: CustomerTemp, value: valueNextProps } = nextProps
        let { listData, valueState, selectedOption } = this.state
        let listDataTemp = [],
            item_selected = ''
        if (list_data_other.length > 0 ) {
            if (valueNextProps) {
                let arrData = valueNextProps.split(',')
                if (valueNextProps.indexOf(',') == -1) {
                    let findItem = list_data_other.filter(x => x.value == valueNextProps)
                    item_selected = { value: findItem[0].code, label: findItem[0].name, name: findItem[0].name, valueParams: findItem[0].valueParams }
            ...
            if (valueState != valueNextProps) {
                this.setState({ listData: listDataTemp, selectedOption: item_selected, valueState: valueNextProps })
            } else {
                this.setState({ listData: listDataTemp, valueState: valueNextProps })
            }
        }
    }
    handleChange(selectedOption) {
        ...
        let list_arr = ''
        this.props.updateBaoGiaCdTurnInOut(selectedOption.value).then(() => {
