...
class ModalLeagueFormView extends React.Component {
  ...
  render() {
    const { leaguesSelect, selects } = this.props.modalLeague || {};
    let { isOpen } = this.props.modal.modalLeague;
    const { hostname } = window.location || {};
    const subDM = SportConfig._getSubdomain(hostname);
    return (
      <div
        className={`modal modal-leaguge ${
          subDM != Config.SUBDOMAIN_ASIA ? "" : "modal-betonline"
        }`}
      ...
      >
        <div className="modal-content">
          ...
          <div className="modal-sub-header text-center">
            ...
            >
              <Translate value="modalLeague.deselectAll" />
            </a>
          </div>
                    ...
                    <div className="margin-bottom-10">
                      <label className="checkbox">
                        <input
                          ...
                          type="checkbox"
                          checked={selects.get(league.id) != false}
                          className={
                            selects.get(league.id) != false
                              ? "check"
                              : "uncheck"
                          }
                        />
          ...
          <div className="modal-footer">
            <div className="text-center">
              <a
                className="button button-accent min-width-200"
                onClick={event => {
                  this._onCancel();
                }}
              >
