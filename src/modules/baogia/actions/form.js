...
export const printBaoGiaDetail = (item) => {
    return (dispatch, getState) => {
        return new Promise((resolve, reject) => {
            axios.post(`${Config.API_URL_USER}baogia/print_detail_baogia`, { Pricecode: item.Pricecode, Info: item }, {
...
export const exportListBangGia = (value = '') => {
    return (dispatch, getState) => {
        return new Promise((resolve, reject) => {
            axios.post(`${Config.API_URL_USER}baogia/export_data`, { page: 1, total: 10000, key: value && value.trim() }, {
                responseType: 'arraybuffer',
                headers: {
                    'Content-Type': 'application/json',
                    'Accept': 'application/xlsx'
...
export const updateCellProductsByOutput = (obj) => {
    return (dispatch, getState) => {
        const { listProductsInProduct, objDataBaoGia } = getState().baogia
        ...
        listProductsInProductTemp = listProductsInProductTemp.map(item => {
            if (item.ProductID == obj.id) {
                ...
                if (obj.key == 'bufferValue') {
                    //((giá cơ sở + buffer) + giá lai) + giá vat
                    //valid integer (positive or negative)
          
                    let { Basicprice, Saleprice, valueLAI, WeightGold, LAI2Value, WeightLAI, BasicPriceWLAI, VATBasicPrice, PriceWVAT, bufferValue } = item
                    if (valueTemp && valueTemp != 0) {
                    } else {
                        valueTemp = 0
                    }
                    // nếu bảng giá fomula ton tai ratio
                    if (objDataBaoGia.PriceType == 3 && objDataBaoGia.Ratio) {
                        let parsePriceBasic = Basicprice + (Basicprice * objDataBaoGia.Ratio) / 100
                        let priceLast = ((parsePriceBasic + parseFloat(valueTemp || 0)) + LAI2Value)
                            + (((Basicprice + parseFloat(valueTemp || 0)) + LAI2Value) * objDataBaoGia.VAT / 100)
                        item.Saleprice = Math.ceil(priceLast / 1000) * 1000
                    } else {
                        let priceLast = ((Basicprice + parseFloat(valueTemp || 0)) + LAI2Value)
                            + (((Basicprice + parseFloat(valueTemp || 0)) + LAI2Value) * objDataBaoGia.VAT / 100)
                        item.Saleprice = Math.ceil(priceLast / 1000) * 1000
                    }
                    item["bufferValue"] = parseFloat(valueTemp)
                } else {
                    item.Saleprice = parseFloat(valueTemp || 1)
                }
            }
...
export const updatePriceBuffer = (item) => {
    return (dispatch, getState) => {
        return new Promise((resolve, reject) => {
            axios.post(`${Config.API_URL_USER}baogia/update_price_buffer`, item)
                .then((response) => {
                    resolve(response)
                }, (err) => {
                    reject(err)
