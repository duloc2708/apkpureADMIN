...
export const updateCellStone = (obj) => {
    return (dispatch, getState) => {
        ...
        if (itemDetailCreateBag.IdOdd == "1") {
            temp.map((item, i) => {
                if (item.IdProductParentIdProductStone == obj.id) {
...
export const saveStoneList = (StatusWeight) => {
    return (dispatch, getState) => {
...
export const saveBagList = (IdOrder, StatusWeight) => {
    return (dispatch, getState) => {
...
export const getListStoneByProductsInBag = (obj, list_data, sttOdd = 0) => {
    console.log('getListStoneByProductsInBag', obj);
                    ...
                    if (obj.IdOdd != "1") {
                        listItemCreateBag.map((obj2, ii) => {
                            ...
                            data.map((item, i) => {
                                ...
                                item_temp.IdProductParentIdProductStone = obj.IdProduct + IdProduct_temp + item.value + item_temp.Color
...
export const getListDataBagBySearch = (value = '') => {
    return (dispatch, getState) => {
        return new Promise((resolve, reject) => {
            axios.get(`${Config.API_URL_USER}bag/search`, { page: 1, total: 10, key: value })
                .then((response) => {
                    let { data } = response.data
                    let data_temp = data
                    dispatch({
...
export const SplitStone = (value) => {
    ...
    return (dispatch, getState) => {
        let { itemStoneSplit, list_stone_save_split, list_stone_save, itemDetailCreateBag, listItemCreateBag } = getState().bag
        const sumsTotalQty = [
        ...
        let list_stone_save_split_temp = []
...
export const removeStoneSplit = (obj) => {
    return (dispatch, getState) => {
        ...
        list_temp.filter(x => x.IdProductColorParentColorStone == obj.IdProductColorParentColorStones).map((item) => {
        ...
        let itemStoneSplit_temp_custom = _.clone(itemStoneSplit, true)
        let total = 0
        list_temp_custom = list_temp_custom.filter(x => x.SplitIdProductColorParentColorStone != obj.SplitIdProductColorParentColorStone)
        list_temp_custom.filter(x => x.IdProductColorParentColorStone == obj.IdProductColorParentColorStones).map((item) => {
            total = total + parseFloat(item.sl2)
        })
        if (list_temp_custom.length == 0) {
            itemStoneSplit_temp_custom.sl2 = itemStoneSplit_temp_custom.remain
        } else {
            itemStoneSplit_temp_custom.sl2 = itemStoneSplit_temp_custom.remain - total
        }

        list_stone_save.map((item) => {
            if (item.IdProductColorParentColorStone == itemStoneSplit_temp.IdProductColorParentColorStone) {
