...
class ListStone extends React.Component {
    ...
    _checkClickRow(item) {
        ...
        if (check) {
            this.props.updatePrimaryStone(item)
        }
        else{
                    ...
                    <tbody>
                        {list_stone_save.map((item, i) => {
                            let { IdStone, Color, value, Weight, PrimaryStone, IdProduct, IdProductColorStone,
                                ...
                                TypeStone, AvgStone, IdProductParentIdProductStone, TypeSplitStone,sl } = item
                            AvgStone = Weight / sl2
                            ...
                            return (
                                <tr key={`dataDetail_${i}`}>
                                    ...
                                    <td><input type="checkbox" checked={valPrimary} onChange={() => this._checkClickRow(item)} /></td>
                                    <td style={{ "textAlign": "left", "fontWeight": PrimaryStone == 1 ? 'bold' : '' }}>
                                        {IdStone}
                                    </td>
                                    ...
                                    <Cell idTable={idTable} index={i} type="text" width="100px" value={Weight || ''} keyInput="value" id={itemDetailCreateBag.IdOdd == "1" ? IdProductParentIdProductStone : IdProductColorStone} parentObject={this} />
                                    <td style={{ "textAlign": "left" }}>
                                        {Exchange}
                                    </td>
                                    <td style={{ "textAlign": "left" }}>
