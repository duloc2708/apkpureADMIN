...
class TabCreateBag extends React.Component {
                ...
                if (checkConfirm) {
                    ...
                    if ([1, 2].indexOf(StatusSplitBag) != -1) {
                        ...
                        // trường hợp tạo bag bộ, lẻ 
                            ...
                            listItemCreateBagTemp.map((itemData, i) => {
                                this.props.updateStatusBagNotSplit(itemData, 2).then(res => {
                                    if (i == listItemCreateBagTemp.length - 1) {
                                        this._getNumberAutoBag(isQuick)
                                    }
                                })
    ...
    _getNumberAutoBag(isQuick) {
        this.props.getNumberAutoBag().then((data) => {
            ...
            this.props.getListStoneByProductsInBagNew(data, listItemCreateBag).then(() => {
                if (isQuick) {
                    this.saveBag(isQuick)
                } else {
                    this.props.showModal(true)
    ...
    saveBag(isQuick) {
        ...
        if (split_odd) {
            ...
            if (IdBagCreate) {
                ...
                let TypeSplitStone = listItemCreateBag[0].TypeSplitStone || 'TYPE_SPLIT_STONE_2'
                ...
                this.props.generateIdBagSplitOdd().then((response) => {
                    ...
                    if (IdTemp) {
                        ...
                        listItemCreateBag_clone = listItemCreateBag_clone.map(item => {
