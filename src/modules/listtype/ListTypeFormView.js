...
class ListTypeFormView extends React.Component {
    constructor(props) {
        super(props);
        this.state = {
            result: []
    ...
    componentWillUnmount() {
        this.props.clearDataListType()
    }
    AddNew() {
        this.props.openModalDetailChuyenMuc(true)
    };
    ...
    editButton(cell, row, enumObject, rowIndex) {
        return (
            <button className="btn btn-warning btn-icon"
                type="button"
                onClick={() => this.props.editInputItemListType(row)} >
                sửa
            </button>
        )
    }
    onDeleteProductSelected(row) {
        ...
        if (r == true) {
            this.props.deleteChuyenMuc(row.id).then(res => {
    ...
    render() {
        ...
        let { list_data, listHeader, isOpen } = this.props.listtype
        return (
                                ...
                                <tbody>
                                    {
                                        list_data.map((item, i) => {
                                            let { id, name, code, slug, numOrder } = item
                                            return (
                                                <tr key={id}>
                                                    <td>{numOrder}</td>
                                                    <td>{code}</td>
                                                    ...
                                                    <td>{slug}</td>
                                                    <td><button
                                                        onClick={() => this.props.editInputItemListType(item)}
...
export default ReactRedux.connect(mapStateToProps, mapDispatchToProps)(ListTypeFormView)
