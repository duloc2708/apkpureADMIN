class Combobox extends React.Component {
  ...
  _onChangeValue(e) {
    ...
    this.props.parentObject.ChangeValueCombobox({
      id: id,
  ...
  componentDidMount() {
    ...
    let { list_data_all } = this.props.init;
    if (data_order && data_order.length > 0) {
      ...
      if (typecb) {
        ...
        let list_data_all_temp = list_data_all.filter(
          x => x.group == type_code
        );
      ...
      >
        {listData &&
          listData.map((item, i) => {
            ...
            return (
              <option key={`${type_code + i}`} value={code}>
                {name}
              </option>
            );
          })}
...
const mapStateToProps = ({ header, i18n, init }, ownProps) => {
  return {
    header,
    i18n,
    init
